...
short Tokenizer::loadDistrubuter()
{
	...
	/*
	// check if the current core is underloaded
	if(this->core->ind_Inbox.size() + this->core->inbox.size() < THRESHOLD)
		return coreID;
	*/
	...
	{	  
		if(this->coreList[crId]->total_inbox_size<MIN)
		{
			MIN = this->coreList[crId]->total_inbox_size;
			coreID = crId;
		}
	}
...
short selectVictim(short list[NIEGHBOORS], Core *core)
{
	...
	for (int i = 0; i < NIEGHBOORS; i++)
	{
		...
		unsigned int inbxSize = core->tokenizer.coreList[coreIDx]->get_ind_size();
		if(inbxSize > MAX)
		{
			MAX = inbxSize;
			victim = coreIDx;
...
void ContextManager::bind_save(Token_Type &tok, int* destAdd, int* retAdd, short &binds, short rest, unsigned long &instIdx)
{
	...
	if(binds > 1)
	{
		...
		if(this->contextMap.find(tok.tag->tokenID) == contextMap.end())
		{
			...
			contextMapContent csMapContent = {new_cx, (short)(binds-1), newCoreId};
...
/* 
	...
	- Save the old cx together with the retAdd, chunk, port, restores & old execution core
...
void ContextManager::restore(Token_Type &tok)
{
	...
	*/	
	...
	// means that this token was sent to us from other core id, then 
